...
<head>
    ...
    <title>@yield('title', 'Gestion Bulletin CPET')</title>
    ...
    <div class="app-container" id="appContainer">
        ...
        <aside class="sidebar" id="sidebar">
            ...
            <div class="sidebar-brand">
                <img src="{{ asset('images/logo.jpeg') }}" alt="Logo CPET" onerror="this.src='{{ asset('images/logo.png') }}'">
            ...
            <nav class="sidebar-nav">
                ...
                @auth
                    @if(auth()->user()->hasPermissionTo('gerer_utilisateurs'))
                    ...
                    @if(auth()->user()->hasPermissionTo('gerer_apprenants'))
                        <a href="{{ route('apprenants.index') }}" class="nav-link {{ request()->routeIs('apprenants.*') ? 'active' : '' }}">
                            <svg fill="currentColor" viewBox="0 0 20 20">
                    ...
                    @if(auth()->user()->hasPermissionTo('gerer_classes_filieres'))
                        ...
                        <a href="{{ route('matieres.index') }}" class="nav-link {{ request()->routeIs('matieres.*') ? 'active' : '' }}">
                            <svg fill="currentColor" viewBox="0 0 20 20">
                    ...
                    @if(auth()->user()->hasPermissionTo('consulter_propres_notes'))
                        <a href="{{ route('my-notes.index') }}" class="nav-link {{ request()->routeIs('my-notes.*') ? 'active' : '' }}">
                            <svg fill="currentColor" viewBox="0 0 20 20">
                    ...
                    @if(auth()->user()->hasPermissionTo('generer_bulletins_pdf'))
                        <a href="{{ route('bulletins.index') }}" class="nav-link {{ request()->routeIs('bulletins.*') ? 'active' : '' }}">
                            <svg fill="currentColor" viewBox="0 0 20 20">
                ...
                <footer class="page-footer">
                    <div class="footer-content">
                        ...
                        <div class="footer-nav">
                            @auth
                                <!-- Section Gestion -->
                                @if(auth()->user()->hasPermissionTo('gerer_apprenants') || auth()->user()->hasPermissionTo('gerer_classes_filieres'))
                                ...
                                <!-- Section Notes -->
                                @if(auth()->user()->hasPermissionTo('saisir_notes') || auth()->user()->hasPermissionTo('consulter_propres_notes'))
                                    <div class="footer-nav-section">
                                        <h3>Notes</h3>
                                        @if(auth()->user()->hasPermissionTo('saisir_notes'))
                                            <a href="{{ route('notes.index') }}">Saisir Notes</a>
                                        @endif
                                        @if(auth()->user()->hasPermissionTo('consulter_propres_notes'))
                                ...
                                <!-- Section Autres -->
                                @if(auth()->user()->hasPermissionTo('enregistrer_absences') || auth()->user()->hasPermissionTo('generer_bulletins_pdf') || auth()->user()->hasPermissionTo('consulter_statistiques'))
                                    <div class="footer-nav-section">
                                        <h3>Autres</h3>
                                        @if(auth()->user()->hasPermissionTo('enregistrer_absences'))
