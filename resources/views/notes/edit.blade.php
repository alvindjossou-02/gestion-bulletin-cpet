            ...
            <div class="p-6 text-gray-900 dark:text-gray-100">
                ...
                <form method="POST" action="{{ route('notes.update', $note) }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="type_evaluation" id="type_evaluation" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:bg-gray-700 dark:border-gray-600" required>
                            @foreach($typeEvaluations as $type)
                                <option value="{{ $type }}" {{ $note->type_evaluation == $type ? 'selected' : '' }}>{{ $type }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="note" id="note" min="0" max="20" step="0.5" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:bg-gray-700 dark:border-gray-600" required value="{{ $note->note }}">
                    </div>
